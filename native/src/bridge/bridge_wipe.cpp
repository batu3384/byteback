...
Napi::Value SetBitLockerRecoveryPassword(const Napi::CallbackInfo& info) {
    ...
    auto unlocked = byteback::unlockBitLockerWithRecoveryPassword(reader, password, 0);
    if (!unlocked.success) {
...
Napi::Value SetBitLockerPassword(const Napi::CallbackInfo& info) {
    ...
    auto unlocked = byteback::unlockBitLockerWithPassword(reader, password, 0);
    if (!unlocked.success) {
        return Napi::String::New(env, unlocked.error);
    }
    if (!reader.setXtsFvek(unlocked.fvek.data(), unlocked.fvek.size())) {
...
public:
    ...
    void Execute() override {
        try {
            ...
            std::string err;
            if (!byteback::loadRecoverRecord(engine_->getMetadataStore(), scanId_, fileId_, rec, err)) {
    ...
    void Execute() override {
        try {
            ...
            for (int64_t id : fileIds_) {
                byteback::FileRecord rec;
                std::string err;
                byteback::RecoveryResult one;
                if (!byteback::loadRecoverRecord(engine_->getMetadataStore(), scanId_, id, rec, err)) {
                    one.success = false;
                    one.error = err;
...
Napi::Value ReadFilePreview(const Napi::CallbackInfo& info) {
    ...
    byteback::FileRecord rec;
    std::string err;
    if (!byteback::loadRecoverRecord(bdata->engine.getMetadataStore(), scanId, fileId, rec, err)) {
        Napi::Object out = Napi::Object::New(env);
        out.Set("success", Napi::Boolean::New(env, false));
