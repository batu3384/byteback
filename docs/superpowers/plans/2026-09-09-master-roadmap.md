...
### 0.2 Saha test paketi yardımcıları - [x]
...
### 0.3 Donanım write-blocker beyanı - [x]
...
### 1.1 NTFS akışkan emit (full-RAM birikimin kaldırılması) - [x]
...
**Doğrulama:** mevcut NTFS testleri aynen + üç yeni: (i) `StreamingEmitCountEqualsMftIndexEntries`; (ii) `StreamingScanPeakMemoryBelowBoundAt200K` (<150MB) ve in-suite `StreamingScanPeakMemoryBelowBoundAt2M` (<700MB, 2026-09: 36.77s PASS); 200K tavanı ×10 = 1500MB > 700MB, büyük-sabit lineer `tempFiles` reddedilir; (iii) `DedupIndex.LoadFromRecordsHydratesResumeDedup`.
...
### 1.3 Sanallaştırılmış sonuç tablosu - [x]
...
### 2.1 NTFS $INDEX_ALLOCATION recarve - [x]
...
### 2.2 ext4 journal (jbd2) replay - [x]
...
### 2.3 HFS+ unused catalog slot'ları - [x]
...
### 2.4 APFS deleted omap/snapshot taraması - [x]
...
### 2.5 XFS silinmiş inode + CRC32C - [x]
...
### 2.6 Metadata kaynaklarında content-hash - [x]
...
### 3.1 RAW koşullu imzalar: NEF/ARW/DNG - [x]
...
### 3.6 RAID5 şerit boyutu kesinleştirme - [x]
...
### 4.1 Hex editör disk-geneli arama - [x]
...
### 4.4 Dosya onarım (kurtarma-sonrası opsiyonel adım) - [x]
...
### 4.5 İnceleyici imaj tara (RAW/E01/VHD) - [x]
Dashboard `pick-scan-image` + native `drivePath "image"` / `kScanImageDriveIndex -2`. `ScanTarget.imagePath` → `attachEvidenceImage` (RAW/E01/VHD ailesi). Scan `volumePath` olarak persist; recover/hex/preview/content-search aynı dosyayı yeniden bağlar. VHD expand tavanı 64 MiB (dürüst). TRIM ack yok (dosya). Yönetici izni gerekmez.
**Doğrulama:** `RawImagePathFindsFat16TestTxt` + `RecoversFat16FromBoundImagePath`.
**Boyut:** M.
...
### 5.5 Kararlılık kararları - [x]
(a) Auto-update: adli araç + offline kullanım → **kapalı** (electron-updater yok); README Updates. (b) Yerel crash-dump: `app.setPath('crashDumps')` + `crashReporter.start({ uploadToServer: false })`; son `.dmp` Dashboard + CaseView. Telemetri yok.
**Doğrulama:** `latestCrashDump` vitest (mtime, boş dizin).
**Boyut:** S.

### 5.6 Sürüm 1.0 ölçütleri - [x]
`docs/release-checklist.md`: faz kapıları + saha A/B/C + imza + installer + CHANGELOG. Dosya var; **satırlar işaretli değil** — v1.0.0 iddiası yok. v0.1.0 kalır.
**Boyut:** S.
