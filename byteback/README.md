...
```
disk/                          # Repo kökü (README.md burada)
├── .github/workflows/         # CI: typecheck + Windows native/Electron/vitest
├── docs/                      # Denetim raporları, yol haritası planları
└── byteback/                  # Uygulama (bu dizin)
    ├── docs/ARCHITECTURE.md   # Motor mimarisi
    ├── native/
    ├── src/
    ├── package.json
    └── README.md
...
2. **Faz 7 (inceleyici yüzeyi)** — Case/NSRL sayfası, ATA ACS etiketi,
   README/ctest hizası.
3. **Denetim** — `docs/codebase-audit/` (repo kökü) tarihli raporlar yaşayan belgedir.
