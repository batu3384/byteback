...
double uniformEntropy() {
    ...
    return byteback::math::calculateEntropy(buf.data(), buf.size());
}
} // namespace

TEST(Entropy, EmptyBufferReturnsZero) {
    EXPECT_DOUBLE_EQ(byteback::math::calculateEntropy(nullptr, 0), 0.0);
    std::vector<uint8_t> empty;
    EXPECT_DOUBLE_EQ(byteback::math::calculateEntropy(empty.data(), 0), 0.0);
}

TEST(Entropy, ConstantBufferReturnsZero) {
    std::vector<uint8_t> zeros(1024, 0);
    EXPECT_DOUBLE_EQ(byteback::math::calculateEntropy(zeros.data(), zeros.size()), 0.0);

    std::vector<uint8_t> fixed(512, 0x42);
    EXPECT_DOUBLE_EQ(byteback::math::calculateEntropy(fixed.data(), fixed.size()), 0.0);
}
...
TEST(Entropy, TwoSymbolUniformIsOneBit) {
    ...
    EXPECT_NEAR(byteback::math::calculateEntropy(buf.data(), buf.size()), 1.0, 1e-6);
}
...
TEST(Entropy, IsBoundedInZeroEight) {
    ...
    double e = byteback::math::calculateEntropy(pseudo.data(), pseudo.size());
    EXPECT_GE(e, 0.0);
    EXPECT_LE(e, 8.0);
}
