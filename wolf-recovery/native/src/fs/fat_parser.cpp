...
#pragma pack(pop)
    ...
    bool scan(DiskReader& reader, FileRecordCallback callback) override {
        ...
        // Basic signature check (very simplified)
        ...
        if (reader.readSectors(rootSector, vbr->sectorsPerCluster, clusterBuf.data())) {
            for (size_t offset = 0; offset < clusterBuf.size(); offset += 32) {
                ...
                if (entry->name[0] == 0xE5) { // Deleted file
                    FileRecord fr;
                    fr.id = "fat_" + std::to_string(offset);
...
FileSystemParser* createFATParser() {
    return new FAT32Parser();
}
