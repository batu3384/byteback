...
void scanUtf16Hints(const uint8_t* data, uint32_t len, int confidence,
    ...
    int hints = 0;
    for (uint32_t i = 0; i + 8 < len && hints < maxHints; i += 2) {
...
void scanRcrdRecords(const uint8_t* data, uint32_t len, int confidence,
                     ...
                     std::atomic<bool>* isRunning, int maxHints) {
    int hints = 0;
    for (uint32_t i = 0; i + sizeof(LogRecordHeader) < len && hints < maxHints; ++i) {
        if (isRunning && !(*isRunning)) break;
...
void scanNtfsLogFileHints(DiskReader& reader, uint64_t partitionOffsetBytes,
    ...
    const uint32_t kScanMax = 4 * 1024 * 1024;
    std::vector<uint8_t> logBuf;
    if (!collectUnnamedData(mftRec.data(), mftSize, reader, volumeStartSector, sectorSize, spc,
                            logBuf, kScanMax)) {
        logBuf.resize(kScanMax);
        if (!reader.readSectors(logRecordOffset + mftSize, kScanMax, logBuf.data()).success) return;
    }

    std::unordered_set<std::string> seen;
    const int kMaxHints = 500;

    // v2: structural RCRD client-data scan (higher confidence).
    scanRcrdRecords(logBuf.data(), static_cast<uint32_t>(logBuf.size()), 58,
                    callback, seen, isRunning, kMaxHints);

    // Fallback: whole-buffer UTF-16 grep for slack outside RCRD records.
