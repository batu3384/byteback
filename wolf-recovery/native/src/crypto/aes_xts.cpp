...
bool xtsAes128Crypt(const uint8_t key32[32], const uint8_t tweak[16],
    ...
    (void)encrypt; // encrypt-only; decrypt needs InvAES (BitLocker unlock is separate)
    uint8_t t[16];
    ...
    for (size_t off = 0; off < len; off += 16) {
        ...
        uint8_t enc[16];
        aes128EncryptBlock(key32, block, enc);
        for (int i = 0; i < 16; ++i) out[off + i] = static_cast<uint8_t>(enc[i] ^ t[i]);
        xtsGfMul(t);
    }
    return true;
}
