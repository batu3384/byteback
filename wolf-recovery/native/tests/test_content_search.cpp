...
protected:
    void SetUp() override {
        path_ = (std::filesystem::temp_directory_path() / "wolf_content_search_test.db").string();
...
TEST_F(ContentSearchTest, ContentFtsFindsIndexedText) {
    ...
    ASSERT_TRUE(store_.upsertContentSample(scanId, fileId, "hello wolf recovery content"));
    auto ids = store_.searchContentFts(scanId, "wolf", 0, 10);
